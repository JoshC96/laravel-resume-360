...
class StorePromptTemplateRequest extends FormRequest
{
    ...
    {
        return [
            self::REQUEST_CONTENT => ['string', 'required'],
            self::REQUEST_USE_LOCATION => ['int', 'required', Rule::in(array_column(PromptTemplateLocation::cases(), 'value'))],
            self::REQUEST_FIELD_STATUS => ['int', 'required', Rule::in(array_column(PromptTemplateStatus::cases(), 'value')) ],
        ];
    }
}
